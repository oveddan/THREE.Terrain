...
export function Analyze(mesh: Mesh, options: TerrainOptions) {
  ...
  let sortNumeric = function (a: number, b: number) { return a - b; },
      ...
      for (let n = -1; n <= 1; n++) {
        for (let m = -1; m <= 1; m++) {
          if (ii + m >= 0 && j + n >= 0 && ii + m < xl && j + n < yl && !(n === 0 && m === 0)) {
            let val = geometry.vertices[(j + n) * xl + ii + m].z;
            sum += val;
            c++;
  ...
  return {
    ...
    fittedPlane: {
      ...
      pctExplained: percentletiationExplainedByFittedPlane(
        geometry.vertices,
        centroid,
        fittedPlaneNormal,
...
function drawHistogram(buckets: number[][], canvas: HTMLCanvasElement, minV: number, maxV: number, append: string = '') {
  ...
  for (i = 0; i < l; i++) {
    let text = Math.floor(((i + 0.5) / buckets.length) * (max - min) + min) + '' + append;
    context!.fillText(
      text,
 ...
 *   explains the letiation in terrain elevation. 1 means entirely explained; 0
 *   means not explained at all.
 */
function percentletiationExplainedByFittedPlane(vertices: Vector3[], centroid: Vector3, normal: Vector3, range: number) {
  let numVertices = vertices.length,
    diff = 0;
  for (let i = 0; i < numVertices; i++) {
    let fittedZ = Math.sqrt(
